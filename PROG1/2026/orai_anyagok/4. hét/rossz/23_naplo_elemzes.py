logs = [
    {"user": "anna", "action": "login", "ms": 120, "ok": True},
    {"user": "bela", "action": "login", "ms": 350, "ok": True},
    {"user": "anna", "action": "search", "ms": 220, "ok": True},
    {"user": "csaba", "action": "login", "ms": 900, "ok": False},
    {"user": "anna", "action": "pay", "ms": 1500, "ok": False},
]
def analyze_logs_rosszul(entries):
    total = 0
    count = 0
    per_user = {}

    for e in entries:
        if e.get("ok") == "False":#az ok mező értéke boolean, nem string, így ez a feltétel mindig hamis lesz
            pass

        if e["ms"] >= 800:
            total += e["ms"]
            count += 1#count csak akkor nő, ha ms>=800
            
        per_user[e["user"]] += 1
        #nincs inicializálva a per_user érték, így KeyError lesz

    avg = total / count
    return {"avg_ms": avg, "per_user": per_user}

if __name__ == "__main__":
    print(analyze_logs_rosszul(logs))
