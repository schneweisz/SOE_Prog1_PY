...
class Bank:
    ...
    def deposit(self, name: str, amount: int) -> None:
        ...
        self.accounts[name] += amount - self.transaction_fee
    ...
    def withdraw(self, name: str, amount: int) -> None:
        ...
        if self.accounts[name] < amount + self.transaction_fee:
            raise ValueError("Insufficient funds for withdrawal and transaction fee.")
        self.accounts[name] -= amount + self.transaction_fee
    ...
    def transfer(self, sender: str, beneficiary: str, amount: int) -> None:
        ...
        if sender not in self.accounts:
            raise ValueError(f"No account found with name '{sender}'.")
        if beneficiary not in self.accounts:
            raise ValueError(f"No account found with name '{beneficiary}'.")
        if amount <= 0:
            raise ValueError("Transfer amount must be positive.")
        if self.accounts[sender] < amount + self.transaction_fee:
            raise ValueError("Insufficient funds for transfer and transaction fee.")
        self.accounts[sender] -= amount + self.transaction_fee
        self.accounts[beneficiary] += amount
    ...
    def show_balances(self, description: str) -> None:
        """Simple utility method to display the balance of all clients using the
        `clients` and `balance` methods on a bar plot accompanied with the description
        provided.
...
def check_step(shouldpass: bool, action, description: str):
    """Simple utility function to test the actions.

    It runs the action, checks if it raises an error, and prints in green/red whether
    the expected behavior is observed or not.
    ...
    try:
        ...
        print("[ MISSING ]", TEST_FAILED)


def test_bank():
    bank: Bank = Bank(transaction_fee=2)
    check_step(True, lambda: bank.create_account("Huey"), "register Huey")
    check_step(False, lambda: bank.create_account("Huey"), "register Huey again")
    check_step(True, lambda: bank.create_account("Dewey"), "register Dewey")
    check_step(True, lambda: bank.create_account("Louie"), "register Louie")
    bank.show_balances("Balances after registration, all three should have 0.")

    check_step(True, lambda: bank.deposit("Huey", 100), "deposit 100 to Huey")
    check_step(False, lambda: bank.deposit("Huey", -10), "deposit -10 to Huey")
    check_step(False, lambda: bank.deposit("Huey", 1), "deposit 1 to Huey")
    check_step(False, lambda: bank.deposit("Donald", 10), "deposit 10 to Donald")
    check_step(True, lambda: bank.deposit("Dewey", 200), "deposit 200 to Dewey")
    check_step(True, lambda: bank.deposit("Louie", 300), "deposit 300 to Louie")
    bank.show_balances("Balances after deposits, should be 98 / 198 / 298")

    check_step(True, lambda: bank.withdraw("Huey", 50), "withdraw 50 from Huey")
    check_step(False, lambda: bank.withdraw("Huey", 50), "withdraw 50 from Huey")
    check_step(False, lambda: bank.withdraw("Dagobert", 1), "withdraw 1 from Dagobert")
    bank.show_balances("Balances after deposits, should be 46 / 198 / 298")

    check_step(
        True,
        lambda: bank.transfer("Dewey", "Huey", 50),
        "transfer 50 from Dewey to Huey",
    )
    check_step(
        False,
        lambda: bank.transfer("Dewey", "Huey", 150),
        "transfer 150 from Dewey to Huey",
    )
    check_step(
        True,
        lambda: bank.transfer("Louie", "Huey", 150),
        "transfer 150 from Louie to Huey",
    )
    ...
    try:
        ...
        print(f"{'Checking final balances':45} [ FAILURE ]", TEST_FAILED, e)
    except NotImplementedError:
        print(f"{'Checking final balances':45} [ MISSING ]", TEST_FAILED)
